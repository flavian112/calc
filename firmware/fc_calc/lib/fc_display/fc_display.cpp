...
static fc_disp_data_t disp_data_draw_buf;
static fc_disp_data_t disp_data_pend_buf;
...
void _draw_menu(fc_menu_labels_t labels) {
  uint8_t box_h = 11;
  ...
  for (int i = 0; i < FC_MENU_LABEL_NUM; ++i) {
    ...
    for (int j = 0; j < FC_MENU_LABEL_SIZE - 1; ++j) {
      if (!labels[i][0]) break;
...
void _draw_lines(fc_line_buf_t lines[4]) {
...
void _draw_flags(fc_flags_t flags) {
  _u8g2.setDrawColor(1);
...
void _draw(fc_disp_data_t *disp_data) {
  _u8g2.clearBuffer();
...
void fc_dp_update(fc_disp_data_t *disp_data) {
  ...
  memcpy(&disp_data_pend_buf, disp_data, sizeof(fc_disp_data_t));
...
void fc_dp_draw(void) {
  ...
  bool redraw = false;
  if (disp_update_pend) {
    memcpy(&disp_data_draw_buf, &disp_data_pend_buf, sizeof(fc_disp_data_t));
    redraw = true;
